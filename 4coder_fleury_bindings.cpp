...
static Command_Map_ID_Pair GlobalCommandMapReroute[3] = {};

CUSTOM_COMMAND_SIG(switch_to_keybinding_0)
CUSTOM_DOC("Switch the keybindings to mode 0.")
{
	GlobalKeybindingMode = KeyBinding_0;
}

CUSTOM_COMMAND_SIG(switch_to_keybinding_1)
CUSTOM_DOC("Switch the keybindings to mode 1.")
{
	GlobalKeybindingMode = KeyBinding_1;
}

function Implicit_Map_Result
casey_implicit_map(Application_Links *app, String_ID lang, String_ID mode, Input_Event *event){
	...
	if(GlobalKeybindingMode == KeyBinding_1)
	{
		for(int PairIndex = 0;
...
F4_SetAbsolutelyNecessaryBindings(Mapping *mapping)
{
	...
	implicit_map_function = casey_implicit_map;
